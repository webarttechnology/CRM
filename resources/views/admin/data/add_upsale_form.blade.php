...
<div class="modal-body">
        ...
        <div class="col-md-12">
          @if ($upsale_data)
          <form method="post" action="{{ route('upsale.update.success') }}" class="form-save">
          <input type="hidden" name="update_id" id="update_id" value="{{ $upsale_data->id }}">
          @else
          <form method="post" action="{{ route('upsale.add.success') }}" class="form-save">
          @endif
          ...
          @csrf
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="client_id" id="client_id" class="form-control" onchange="getproject();">
                        ...
                        <option value="{{ $val['id'] }}"  {{ $val['id'] == $upsale_data?->client_id ? 'Selected':'' }}>{{ $val['name'].' ('.$val['client_code'].')' }}</option>
                        @endforeach    
                    </select>
                ...
                <div class="col-md-6 mb-3">
                        ...
                        <select name="project_id" id="project_id" class="form-control">
                            ...
                            @foreach($project as $val)
                                <option value="{{ $val->id }}" {{ $val->id == $upsale_data?->sale_id ? 'Selected':'' }}>{{$val->project_name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-12 mb-3">
                        ...
                        <select name="upsale_type" id="upsale_type" class="form-control" onchange="projectTypechangeEvent()">
                            ...
                            <option value="{{ $key }}" {{ $key == $upsale_data?->upsale_type ? 'Selected':'' }}>{{ $val }}</option>
                            @endforeach                                   
                        </select>
            ...
            <div class="row" id="div_hosting">
                      <div class="col-md-6 mb-3">
                        ...
                        <input  type="date" name="start_date" id="start_date" class="form-control" value="{{ date("Y-m-d", strtotime($upsale_data?->start_date)) }}">                                        
                      </div>  
                      <div class="col-md-6 mb-3">
                        <label class="form-label" for="end_date">{{ __("End Date") }} <span class="text-danger">*</span></label>
                        <input  type="date" name="end_date" id="end_date" class="form-control" value="{{ date("Y-m-d", strtotime($upsale_data?->end_date)) }}">                                            
                      </div>
            </div>
            ...
            <div class="row" id="div_other">
              ...
              <div class="row">                           
                  <div class="col-md-4 mb-3">
                        ...
                        <input  type="number" name="gross_amt" id="gross_amt" class="form-control pendingamount" placeholder="$" value="{{ $upsale_data?->gross_amount }}">  
                  ...
                  <div class="col-md-4 mb-3">
                        ...
                        <input  type="number" name="net_amt" id="net_amt" class="form-control pendingamount" placeholder="$" value="{{ $upsale_data?->net_amount }}">  
                  ...
                  <div class="col-md-4 mb-3">
                        ...
                        <input  type="text" readonly name="due_amt" id="due_amt" class="form-control" placeholder="$" value="{{ $upsale_data?->gross_amount - $upsale_data?->net_amount }}">              
                  </div>
                  <div class="col-md-6 mb-3">
                        <label class="form-label" for="sale_date">{{ __("Sale Date") }} <span class="text-danger">*</span></label>
                        <input  type="date" name="sale_date" id="sale_date" class="form-control" value="{{ date("Y-m-d", strtotime($upsale_data?->sale_date)) }}">  
                  ...
                  <div class="col-md-6 mb-3">
                        ...
                        <select name="payment_mode" id="payment_mode" class="form-control" onchange="paymentonchangeevent()">
                            ...
                            <option value="{{ $i }}" {{ $i == $upsale_data?->payment_mode ? 'Selected' : '' }}>{{ $val }}</option>
                            @endforeach    
                        </select>
                   ...
                   <div class="col-md-12 mb-3" id="div_other_pay">
                        ...
                        <input  type="text" name="other_payment_mode" id="other_payment_mode" class="form-control" placeholder="Description" vlaue="{{ $upsale_data?->other_payment_mode}}">              
                  </div>
              </div>                     
             <div class="row">
                <div class="col-md-12 text-center">
                    <button type="submit" class="btn btn-primary mt-5 form-submit">{{ $upsale_data ? 'Update' : 'Submit' }}</button>
                </div>
             </div>
          </form>
        </div>
