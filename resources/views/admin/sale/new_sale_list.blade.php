                    ...
                    <div class="col text-end">
                        @if (Auth::user()->role_id != 3)
                            <a href="#"
                                ...
                                data-type="add_sales">Add Sales</a>
                        @endif
                    </div>
                </div>
            </div>
                                            ...
                                            <div class="input-group input-group-merge">
                                                <select name="status" id="status" class="form-control">
                                                    ...
                                                    @foreach (projectstatus() as $val)
                                                        <option value="{{ $val }}"
                                                            {{ $status == $val ? 'Selected' : '' }}>{{ $val }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <form action="{{ route('sales.new.list.success') }}" method="post"
                                        ...
                                        <div class="col-sm-12">
                                            <table
                                                ...
                                                <tbody>
                                                    @foreach ($data as $val)
                                                        <tr
                                                            class={{ Auth::user()->id == 1 ? (getcommentstatus($val->id) == 0 ? 'text-danger' : '') : '' }}>
                                                                ...
                                                                <div class="dropdown dropdown-action">
                                                                    ...
                                                                    <div class="dropdown-menu dropdown-menu-right">
                                                                        <a class="dropdown-item  open-module-form"
                                                                            data-id="{{ $val->id }}"
                                                                            data-type="add_sales"
                                                                            href="{{ route('sales.update', ['updateid' => $val->id]) }}"><i
                                                                                class="bx bx-edit-alt me-1"></i> Edit</a>
                                                                        <a class="dropdown-item"
                                                                            href="{{ route('sales.view', ['salesid' => $val->id]) }}"><i
                                                                                class="fa fa-eye"></i> Show</a>
                                                                        @if (Auth::user()->role_id < 3 || Auth::user()->role_id == 9)
                                                                            <a class="dropdown-item open-module-form"
                                                                                data-id="{{ $val->id }}"
                                                                                data-type="add_sales" data-sale="assign"
                                                                                href="{{ route('sales.assign', ['taskid' => $val->id]) }}"><i
                                                                                    class="bx bx-edit-alt me-1"></i>
                                                                                Assign</a>
                                                                        @endif
                                                                        <a class="dropdown-item open-module-form"
                                                                            data-id="{{ $val->id }}"
                                                                            data-type="add_sales" data-sale="comment"
                                                                        ...
                                                                        @if (Auth::user()->role_id == 1 || Auth::user()->role_id == 9)
                                                                            <a class="dropdown-item"
                                                                                ...
                                                                                href="{{ route('sales.delete', ['deleteid' => $val->id]) }}"><i
                ...
                <div class="modal-body">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form method="post" action="{{ route('sales.client.insert.suceess') }}"
                                class="client-save save">
                                @csrf
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        ...
                                        @if ($errors->has('name'))
                                            <small class="text-danger"
                                                id="nameerrmsg">{{ $errors->first('name') }}</small>
                                        @endif
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <select name="country_name" id="country_name" class="form-control">
                                            ...
                                            @foreach (country() as $val)
                                                <option value="{{ $val['name'] }}"
                                                    {{ old('country_name') == $val['name'] ? 'Selected' : '' }}>
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input type="email" class="form-control" id="email" name="email"
                                            placeholder="jhon.doe@gmail.com" value="{{ old('email') }}" />
                                        @if ($errors->has('email'))
                                            <small class="text-danger"
                                                id="emailerrmsg">{{ $errors->first('email') }}</small>
                                        @endif
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input type="text" class="form-control" id="address" name="address"
                                            placeholder="1A, Ho Chi Minh Sarani Rd" value="{{ old('address') }}" />
                                        @if ($errors->has('email'))
                                            <small class="text-danger"
                                    ...
                                    <div class="col-md-2 mb-3">
                                        ...
                                        <span id="addrow" class="btn btn-primary mt-4"><i
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <label class="form-label"
                                            for="current_website">{{ __('Current Website') }}</label>
                                        <input type="text" class="form-control" id="current_website"
                                            name="current_website" placeholder="webart.technology"
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <label class="form-label" for="agent_name">{{ __('Agent Name') }} <span
                                                class="text-danger">*</span></label>
                                        <input type="text" name="agent_name" id="client_agent_name"
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <label class="form-label" for="closer_name">{{ __('Closer Name') }} <span
                                                class="text-danger">*</span></label>
                                        <input type="text" name="closer_name" id="client_closer_name"
                                            class="form-control" placeholder="Closer name"
                                            value="{{ old('closer_name') }}">
    ...
    <script>
            ...
            $(document).on("click", ".form-submit", function(e) {
                ...
                if ($("#client_id").val() == "") {
                    ...
                    return false;
                        ...
                        'input[type=radio][name=digital_marketing]:checked').length == 0) {
                    ...
                    return false;
                } else if ($("#project_type").val() == 2 && $('input[name^=smo_platfrom]:checked').length ==
                    0 && ($('input[type=radio][name=digital_marketing]:checked').val() == 'SMO' || $(
                        'input[type=radio][name=digital_marketing]:checked').val() == 'SEO_SMO')) {
        ...
        $("#addrow").click(function() {
            markup = '<div class="row" id="deleterow' + lineNo +
                ...
                lineNo +
                '" name="alteremail[]" placeholder="jhon.doe@gmail.com" /></div><div class="col-md-5 mb-3"><label class="form-label" for="address">{{ __('Mobile No') }}</label><input type="text" class="form-control" id="address' +
                lineNo +
                '" name="mobile_no[]" placeholder="1A, Ho Chi Minh Sarani Rd" /></div> <div class="col-md-2 mb-3"> <span class="btn btn-danger mt-4 delete-row"><i class="fa fa-trash-o" aria-hidden="true"></i></span></div> </div>';
        ...
        if ($("#client_id").val() == "") {
            ...
            return false;
        } else if ($("#project_type").val() == 2 && $('input[type=radio][name=digital_marketing]:checked')
            .length == 0) {
            toastr.error('Select one platform atleast!');
            $("#seo").focus();
            $("#seo").css({
            ...
            return false;
        } else if ($("#project_type").val() == 2 && $('input[name^=smo_platfrom]:checked').length == 0 && ($(
                'input[type=radio][name=digital_marketing]:checked').val() == 'SMO' || $(
                'input[type=radio][name=digital_marketing]:checked').val() == 'SEO_SMO')) {
            toastr.error('Select one smo platform atleast!');
            $("#facebook").focus();
            $("#facebook").css({
